...
app.post('/api/sms', async (req, res) => {
    const { raw_text, sender = 'Volunteer_Phone' } = req.body;
    try {
        // Simple NLP parse: FORMAT: "DISEASE VILLAGE CASES" (e.g., "FEVER SHILLONG 3")
        const parts = raw_text.toUpperCase().trim().split(' ');
        let parsed_status = 'Failed';
        if(parts.length >= 2) {
            const disease = parts[0];
            const village = parts[1];
            const casesCount = parseInt(parts.length >= 3 ? parts[2] : "1") || 1;
            
            for(let i=0; i<casesCount; i++) {
                await runDB(`INSERT INTO reports (reporter_name, reporter_role, village, symptoms, disease_suspected, source) VALUES (?, ?, ?, ?, ?, ?)`,
                   [sender, 'Community Volunteer', village, disease, disease, 'SMS']
                );
            }
            parsed_status = 'Success';
        }
        await runDB(`INSERT INTO sms_inbox (raw_text, sender, parsed_status) VALUES (?, ?, ?)`, [raw_text, sender, parsed_status]);
        res.json({ message: 'SMS logic processed', status: parsed_status });
    } catch(err) {
        res.status(500).json({ error: err.message });
    }
});
...
// ==========================================
// 4. ALERTS ENDPOINTS (ACK & ML INJECTION)
// ==========================================
app.post('/api/alerts', async (req, res) => {
    const { type, message, village, severity, risk_score = 99, ai_explanation = "Scikit-Learn ML Flag", source = "ML Engine" } = req.body;
    try {
        await runDB(
            `INSERT INTO alerts (type, message, village, severity, risk_score, ai_explanation, source) VALUES (?, ?, ?, ?, ?, ?, ?)`, 
            [type, message, village, severity, risk_score, ai_explanation, source]
        );
        res.json({ message: 'ML Alert registered successfully' });
    } catch(err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.get('/api/dashboard/summary', async (req, res) => {
    try {
        const summary = { recent_reports: [], sensor_readings: [], alerts: [], village_risks: {}, sms_logs: [] };
        ...
        const sms_logs = await queryDB(`SELECT * FROM sms_inbox ORDER BY timestamp DESC LIMIT 20`);
        
        summary.recent_reports = reports;
        summary.sensor_readings = sensors;
        summary.alerts = existingAlerts;
        summary.sms_logs = sms_logs;

        // --- ENHANCED AI RULE-BASED LOGIC ---
        const villageStats = {};
...
app.listen(PORT, '0.0.0.0', () => { 
    ...
    console.log(`[ML Engine] Spawning Python ML Microservice...`);
    
    const startML = () => {
        const mlProcess = spawn('python', ['../ml-models/detect_anomalies.py']);
        mlProcess.stdout.on('data', (data) => console.log(`[ML-Python]: ${data.toString().trim()}`));
        mlProcess.stderr.on('data', (data) => console.error(`[ML-Error]: ${data.toString().trim()}`));
        mlProcess.on('exit', (code, signal) => {
            console.error(`[ML Engine] Microservice exited (code: ${code}, signal: ${signal}). Restarting in 10s...`);
            setTimeout(startML, 10000);
        });
    };
